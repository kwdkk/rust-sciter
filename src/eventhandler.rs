...
{
	if he.is_null()
		&& evtg != EVENT_GROUPS_SUBSCRIPTIONS_REQUEST
		&& evtg != EVENT_GROUPS::HANDLE_BEHAVIOR_EVENT.bits()
		&& evtg != EVENT_GROUPS::HANDLE_INITIALIZATION.bits()
		&& evtg != EVENT_GROUPS::HANDLE_SOM.bits()
	{
		eprintln!("[sciter] warning! null element for {:04X}", evtg);
	}

	let result = if evtg == EVENT_GROUPS_SUBSCRIPTIONS_REQUEST {
			assert!(!params.is_null());
		...
		let evtg = EVENT_GROUPS::from_bits_truncate(evtg);
